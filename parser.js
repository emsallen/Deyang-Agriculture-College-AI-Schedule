    ...
    trs.slice(2).each(function(_,index){
        ...
        tds.each(function(__,index){
            ...
            kc.forEach(con=>{
                ...
                if(!!con.match(/\(调\d+\)|\(换\d+\)/)) return
                let conInfo = {weeks:[],sections:[]}
                conInfo.name = singleCon[0]
                conInfo.teacher = singleCon[2]
                conInfo.position = singleCon[3]
                conInfo.day = days.indexOf(singleCon[1].slice(1,2))
                conInfo.sections = getSections(singleCon[1].slice(2).split('{')[0])
                conInfo.weeks = getWeeks(singleCon[1].match(/\{.*?\}/)[0])
                result.push(conInfo)
            })
        })
    })
